"""
Curated UI metadata for OneTrainerWeb type generation.

This file is the single source of truth for display labels, tooltips,
and data type subsets that cannot be derived purely from Python enums
or config classes. The generate_types.py script imports this data.

When a new enum value is added to the Python backend, add its display
label here. If no label is provided, the generator uses a smart
fallback (replace underscores with spaces, title-case each word).

Tooltip strings are extracted from the Python UI code (modules/ui/*.py).
Only config-field tooltips are included; button/action tooltips are omitted.
"""

# Enum display labels
# Keyed by enum class name, then enum .value string -> display label.
# This structure avoids collisions when the same value string appears
# in multiple enums with different intended labels.

ENUM_DISPLAY_LABELS: dict[str, dict[str, str]] = {
    "ModelType": {
        # Names match the original TopBar.py dropdown labels
...
# DataType subsets for dropdowns
# Each entry is (display_label, enum_value). Order matters for UI.
...
# Tooltip text for config fields
# Keyed by config field path (dot-notation matching TrainConfig structure).
# Only config-field tooltips are included here.
# Source: modules/ui/*.py tooltip= keyword arguments.
...
FIELD_TOOLTIPS: dict[str, str] = {
    ...
    # ── UNet / Transformer / Prior weight dtypes ──────────────────────────
    ...
    # Text encoder N (indexed) - generic versions
...
# Field keys where wide_tooltip=True in the Python UI
